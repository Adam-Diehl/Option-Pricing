...
File: Prices vanilla (European) options using the Heston model using a
  parallelized Milstein discretization. Uses the built in (BI) standard normal RNG.
...
gcc-7 -o HestonBI VanillaHestonOptionBI.cpp -O3 -lstdc++ -fopenmp
...
/*------------------------ Random Number Generation --------------------------*/
  ...
  for (int i=0; i<N; ++i) {
    for (int j=0; j<NCols; ++j) {
        dW1[i][j] = RNORM(gen);
        dW2[i][j] = dW1[i][j]*Rho + RhoCoefSpec*RNORM(gen);
    }
  }
